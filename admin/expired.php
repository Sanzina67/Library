...
<div class="container">
  ...
  <button name="submit2" type="submit" class="btn btn-default" style="background-color:red;color:yellow">EXPIRED</button>
     ...
     <div class="srch">
    <form method="post" action="">
      <input style="background-color:yellow;color:black;" type="text" name="username" class="form-control" placeholder="Username" required=""><br>
       <input style="background-color:yellow;color:black"type="text" name="bid" class="form-control" placeholder="BOOK_ID" required=""><br>
       <button style="color:black;margin:auto 100px"class="btn btn--default" name="submit3" type="submit">Submit</button>
    </form>
  </div>
  <?php
  if(isset($_POST['submit3']))
  {
    ...
    $ret1='<p style="color:yellow;background-color:green;">RETURNED</p>';
    mysqli_query($db,"UPDATE issue_book SET approve='$ret1' where username='$_POST[username]' and bid='$_POST[bid]' ");
  ...
  if(isset($_SESSION['login_user']))
  {
      ...
      {
     $sql="SELECT user_reg.username,user_id,books.bid,name,authors,status,edition,approve,issue_date,return_date from user_reg inner join issue_book ON user_reg.username=issue_book.username inner join books ON books.bid=issue_book.bid WHERE issue_book.approve ='$ret' ORDER BY `issue_book`.`return_date` ASC ";
     $res=mysqli_query($db,$sql);
       }
      else if(isset($_POST['submit2']))
    {
     $sql="SELECT user_reg.username,user_id,books.bid,name,authors,status,edition,approve,issue_date,return_date from user_reg inner join issue_book ON user_reg.username=issue_book.username inner join books ON books.bid=issue_book.bid WHERE issue_book.approve ='$exp' ORDER BY `issue_book`.`return_date` DESC  ";
     $res=mysqli_query($db,$sql);
   }
    else
    {
     $sql="SELECT user_reg.username,user_id,books.bid,name,authors,status,edition,approve,issue_date,return_date from user_reg inner join issue_book ON user_reg.username=issue_book.username inner join books ON books.bid=issue_book.bid WHERE issue_book.approve !='' and issue_book.approve !='YES' ORDER BY `issue_book`.`return_date` DESC  ";
     $res=mysqli_query($db,$sql);
   }
